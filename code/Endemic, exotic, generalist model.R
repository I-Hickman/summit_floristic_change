...
CGD <- read.csv("data/Summit CGDD & TSF.csv")

#Put the Florisitc dataframe into long form
Florisitc <- Florsitics %>% 
  gather(key = "Species", value = "cover", 5:107)

#Join SR to florisitics
Flor_SR <- Florisitc %>% 
  left_join(Traits, by = "Species") %>% 
  left_join(CGD, by = c("Site", "Year"))

#check where the NAs are and put them in a dataframe
NAs <- Flor_SR %>% 
  filter(is.na(Origin))
...
Florisitc_LO <- Flor_SR %>% 
  ...
  gather(key = "Species", value = "Cover", 14:107) 
...
### Plot
EGE_plot <- ggplot(new_end1, aes(x = SR.w.m2, y = mean)) + 
  geom_path(color = "darkgreen") +
  geom_ribbon(aes(ymin = lci, ymax = uci), alpha = 0.2, fill = "darkgreen") +
  #Generalist species
  geom_path(data = new_gen1, aes(x = SR.w.m2, y = mean), color = "darkblue") +
  geom_ribbon(data = new_gen1, aes(ymin = lci, ymax = uci), alpha = 0.2, fill = "darkblue") +
  #Exotic spcies
  geom_path(data = new_exotic1, aes(x = SR.w.m2, y = mean), color = "darkorange") +
  geom_ribbon(data = new_exotic1, aes(ymin = lci, ymax = uci), alpha = 0.2, fill = "darkorange") +
  theme_cowplot() +
...
EG_cov_data <- Flor_traits %>% 
  ...
  dplyr::select(Code, Site, SR = avg.SR, ele, Aspect, Quadrat,
                Species, Cover, Origin) %>%
  dplyr::mutate(pCover = ((Cover/100)+0.001)) #turn % cover into proportional data
